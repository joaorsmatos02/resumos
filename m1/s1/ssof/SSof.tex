...
In Unix, each user has a username and a user ID (UID) and belongs to one or more groups, each with a group ID (GID).\\
\\
Each object has its owner UID and GID and specific access permissions (rwx), for owner, group and world. When objects are accessed by processes, the effective UID (EUID) and the effective GID (EGID) are compared with the object permissions to grant/deny access.\\
\\
There are also two more access bits: setuid and setgid. When the setuid/setgid bit is set on an executable file, the program is executed with the effective user ID / group ID of the file's owner. Privilege escalation attacks in Unix often aim programs with setuid and owner UID 0.
